...
export class FastBezier2 {
  ...
  static bezierPointForQuadrant(t, quadrant) {  
    ...
    switch(quadrant) {
      case QUADRANTS.Q1:
        pt = FastBezier2.bezierPoint(1 - t);
        pt.x = -pt.x;
        pt.y = -pt.y;
        return pt;
      case QUADRANTS.Q2:
        pt = FastBezier2.bezierPoint(t);
        pt.y = -pt.y;
        return pt;
      case QUADRANTS.Q3:
        return FastBezier2.bezierPoint(1 - t);
      case QUADRANTS.Q4: 
        pt = FastBezier2.bezierPoint(t)
        pt.x = -pt.x;
        return pt;
  ...
  */
  static bezierPointSet(numQuadrantPoints) {
    ...
    for(let t = 0; t < .9999; t += t_increment) {
      BezierQ1.push(FastBezier2.bezierPointForQuadrant(t, QUADRANTS.Q1));
      BezierQ2.push(FastBezier2.bezierPointForQuadrant(t, QUADRANTS.Q2));
      BezierQ3.push(FastBezier2.bezierPointForQuadrant(t, QUADRANTS.Q3));
      BezierQ4.push(FastBezier2.bezierPointForQuadrant(t, QUADRANTS.Q4));
    }
    const Bezier360 = []
  ...
  */
  ...
  cachePointSet(numQuadrantPoints) {
    this.cachedPointSets.set(numQuadrantPoints, 
      FastBezier2.bezierPointSet(numQuadrantPoints));
  }
  ...
  */ 
  bezierPadding(r0, r1, numQuadrantPoints, radius = r0.distance) {
    ...
    const start_quadrant = FastBezier2.getQuadrant(r0.B, origin);
    const end_quadrant = FastBezier2.getQuadrant(r1.B, origin);
    ...
    // unscale each point between start and end
    // loop back to beginning if necessary
    const scaled_pts = [];
    const end_i = end_idx > start_idx ? end_idx : bezierPoints.numQuadrantPoints * 4;
    for(let i = start_idx; i < end_i; i += 1) {
      const pt = bezierPoints.Full[i];
      scaled_pts.push({
    ...
    if(end_idx <= start_idx) {
     // start from beginning until end idx
     for(let i = 0; i < end_idx; i += 1) {
       const pt = bezierPoints.Full[i];
       scaled_pts.push({
         x: this.unscale(pt.x, origin.x, radius),
         y: this.unscale(pt.y, origin.y, radius)
...
export const BEZIER2 = new FastBezier2();
BEZIER2.cachePointSet(30); // density / 2 to get numQuadrantPoints to cache
...
export class MyClockwiseSweepPolygon2 extends ClockwiseSweepPolygon {
  ...
  */
  _getPaddingPoints(r0, r1) {
    ...
    const pts = BEZIER2.bezierPadding(r0, r1, 
