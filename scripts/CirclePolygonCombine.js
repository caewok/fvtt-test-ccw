 ...
 * @return {PIXI.Polygon}
 */
export function circle_union(poly, { density = 60, algorithm = 1 } = {}) {
  // for simplicity, if poly is also a circle, just convert to a polygon
  poly = algorithm == 1 ? SimplePolygon.fromPolygon(poly, { density }) : poly;
  ...
  return _combine(poly, this, { clockwise: false, density, algorithm });
}
 ...
 * @return {PIXI.Polygon}
 */
export function circle_intersect(poly, { density = 60, algorithm = 1 } = {}) {
  poly = algorithm == 1 ? SimplePolygon.fromPolygon(poly, { density }) : poly;
  const out = _combine(poly, this, { clockwise: true, density, algorithm });
 ...
 * @private
 */
function _combine(poly, circle, { clockwise = true, density = 60, algorithm = 1 } = {}) {
  const pts = algorithm == 1 ?
              _tracePolygon(poly, circle, { clockwise, density }) :
              _tracePolygon2(poly, circle, { clockwise, density });
...
export function _tracePolygon2(poly, circle, { clockwise = true, density = 60 } = {}) {
...
//     });
    ...
    if(ixs_result.intersections.length == 2) {
      ...
      processIntersection2(circle, edge, ix_data, false);

      ix_data.ix = ixs_result.intersections[1];
      processIntersection2(circle, edge, ix_data, true);
...
function processIntersection2(circle, edge, ix_data, is_second_ix) {
  let { aInside,
          bInside,
          clockwise,
          ix } = ix_data;
...
//     console.log(`${i}: ${tracing_segment ? "tracing" : "not tracing"} segment`);
...
//       console.log(`${circle_x.intersections.length} intersections.`);
...
//         console.log(`Adding edge.B ${curr_edge.B.x},${curr_edge.B.y}`);
...
//     console.log("Moving segment --> circle");
  ...
  if(was_tracing_segment || tracing_segment) {
//     console.log(`Adding intersection ${x.x},${x.y}`);
    pts.push(x.x, x.y);
  }
