...
import { IntersectionsXOrderBST, 
         IntersectionsYOrderBST } from "./class_IntersectionsBST.js";
...
export class CCWSweepPolygon extends PointSourcePolygon {
  ...
  * @return {Wall[]}
  */
  _processWallIntersectionsBruteForce(walls) { 
    // need a Map to avoid repeating intersections
    // id is the combined id of the two walls
    // order will be a problem so we check both combinations
    ...
    return this._buildWallsFromIntersections(intersections.inorder(), walls);
  }
  
 /**
  * Determine if walls intersect one another.
  * Better than brute force version, but still relatively simple
  ...
  * @return {Wall[]}  
  */
  _processWallIntersectionsSimpleSweep(walls) {
    const sorted_walls = [];
    walls.forEach(w => {
      // use SweepWall for intersect, intersection methods
      const out = { wall: CCWSweepWall.create(w) };
      
      // figure out which is left, for sort.
      const coords = w.coords;
      const is_left = coords[0] < coords[2];
      out.left = is_left ? { x: coords[0], y: coords[1] } : { x: coords[2], y: coords[3] };
      out.right = !is_left ? { x: coords[0], y: coords[1] } : { x: coords[2], y: coords[3] }; 
      
      sorted_walls.push(out);
    });
  
    sorted_walls.sort((a, b) => {
      return a.left.x < b.left.x ? -1 :
             a.left.x > b.left.x ? 1 : 0;
    });
    ...
    for(i = 0; i < ln; i += 1) {
      ...
      for(j = (i + 1); j < ln; j += 1) {
        ...
        if(candidate.left.x > test.right.x) { break; }
        
        if(test.wall.intersects(candidate.wall)) {
          const i_point = test.wall.intersectSegment(candidate.wall.coords);
        
          let existing_intersection = intersections.find(i_point);
          if(existing_intersection) {
            // update with the additional wall
            existing_intersection.data.walls.set(test.wall.id, test.wall);
            existing_intersection.data.walls.set(candidate.wall.id, candidate.wall);
            ...
            existing_intersection.walls.set(test.wall.id, test.wall);
            existing_intersection.walls.set(candidate.wall.id, candidate.wall);
            intersections.insert(existing_intersection);
          }
        }
      }
  ...
  * @private
  */
  _buildWallsFromIntersections(intersections, walls) {
    ...
    walls.forEach(w => {
      ...
      // sort the intersections by x then y
      // TO-DO: can we skip b/c the tree already sorts these?
      intersection_objs.sort((a, b) => {
        if(a.x === b.x) { 
          return a.y < b.y ? -1 :
                 a.y > b.y ? 1 : 0;
        }
      ...
      const is_left = w.coords[0] < w.coords[2] ? true :
                      w.coords[0] > w.coords[2] ? false :
                      w.coords[1] < w.coords[3] ? true : false;
      let remaining_w;
      if(is_left) {
        remaining_w = CCWSweepWall.createFromPoints({x: w.coords[0], y: w.coords[1] },
                                                    {x: w.coords[2], y: w.coords[3] }, w);                                           
      } else {
        remaining_w = CCWSweepWall.createFromPoints({x: w.coords[2], y: w.coords[3] },
                                                    {x: w.coords[0], y: w.coords[1] }, w);
      }
      ...
      intersection_objs.forEach(obj => {
        ...
        const new_w = CCWSweepWall.createFromPoints(remaining_w.A, { x: obj.x, y: obj.y }, w); 
        remaining_w = CCWSweepWall.createFromPoints({ x: obj.x, y: obj.y }, remaining_w.B, w); 
  ...
  * This method implements a sweep along the x-coordinates.
  * Wall endpoints are either left or right events; intersections are their own event.
  * Each event is a checkpoint for the x-sweep. 
  * When we hit a left event, we check the segments immediately above and below for
  *   intersection with the event segment.
  * When we hit a right event, drop that segment and check the segments immediately
  *   above and below for intersection with each other.
  * When we hit an intersection event (added from left or right events), we
  *   record the intersection and swap the above/below segments. 
  *   (Think of a cross, where the above/below segments switch position at the intersect)
  * Time: Theoretically, O(n*log(n) + I * log(n)), 
  *       where n = number segments; I = number intersections
  *       But depends on how well the search tree works for this
  *
  * Requires two sorted queues:
  * 1. Event Queue: Set of events, sorted by increasing x-coordinates
  ...
  * @private 
  */
  _processWallIntersectionsComplexSweep(walls) {
      ...
      //    Add events to the queue     
          ...
          if(s1) {  
            ...
            if(intersection1) {
              ...
              intersection1.event = "intersection";
              intersection1.wall0 = e.wall;
              intersection1.wall1 = s1.wall;
          ...
          if(s2) {
            ...
            if(intersection2) {
              ...
              intersection2.event = "intersection";
              intersection2.wall = e.wall;
              intersection2.wall1 = s2.wall
       ...
       // 3. Test intersections between s' and s''. Add to event queue
          ...
          if(intersection) {
            ...
            intersection.wall0 = s1.wall;
            intersection.wall1 = s2.wall;
    ...
    // for each intersection, build set of new walls
    return this._buildWallsFromIntersections(intersections, walls);
  }
